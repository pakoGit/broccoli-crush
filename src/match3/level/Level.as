package match3.level {
	...
	public class Level {
		...
		public function build(arr:Array):void {
			_colums = arr.length;
			_rows = arr[0].length;
			offsetX = (BroGlobal.camera.width >> 1) - (_colums * GVar.TILE_W >> 1);
			offsetY = (BroGlobal.camera.height >> 1) - (_rows * GVar.TILE_H >> 1);
			x = offsetX;
			y = offsetY;
			for (var i:int = 0; i < _colums; i++) {
				...
				for (var j:int = 0; j < arr[i].length; j++) {
					...
					_chips[i][j] = 0;
					if (tid <= 0) continue;
					var cell:Cell = new Cell(offsetX + i * GVar.TILE_W, offsetY + j * GVar.TILE_H);
		...
		private function onEmiiterReady(x:Number, y:Number):void {
			var i:int = int((x - offsetX) / GVar.TILE_W);
			var j:int = int((y - offsetY) / GVar.TILE_H);
			addChip(i, j);
		}
		
		public function addChip(i:int, j:int):void {
			if (_chips[i][j] != 0) return;
			var chip:Chip = new Chip(offsetX + i * GVar.TILE_W, offsetY + j * GVar.TILE_H, Math.random() * 5 + 1);
